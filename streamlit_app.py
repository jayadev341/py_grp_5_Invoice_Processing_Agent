...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Upload Vendor Invoice", type=["pdf", "png", "jpg", "docx"])
...
if "file_path" in st.session_state:
    ...
    with col1:
        ...
        # =====================================================================
        # INTERACTIVE HUMAN-IN-THE-LOOP OVERRIDE CONTROL ROOM
        # =====================================================================
        if state_info.next:
            ...
            st.info("Reason: Invoice requires explicit manual oversight (Value exceeds $50,000 threshold limit).")
            
            # Create side-by-side buttons for the auditor's course of action
            col_approve, col_reject = st.columns(2)
            
            with col_approve:
                if st.button("✅ Approve & Pass Override", type="primary", use_container_width=True):
                    with st.spinner("Injecting approval token..."):
                        # Idiomatic LangGraph: Mutate the state token, telling it the human cleared the match
                        app.update_state(
                            config, 
                            {"routing_decision": "Matched"}, 
                            as_node="human_review"
                        )
                        # Resume execution thread stream
            ...
            with col_reject:
                if st.button("❌ Issue Audit Rejection", type="secondary", use_container_width=True):
                    with st.spinner("Injecting rejection token..."):
                        # Idiomatic LangGraph: Overwrite routing decision and append the manual violation text
                        app.update_state(
                            config, 
                            {
                                "routing_decision": "Exception",
                                "validation_errors": ["Compliance Rejection: Manually blocked by senior financial auditor review team."]
                            }, 
                            as_node="human_review"
                        )
                        # Resume execution thread stream
...
"""
            ...
            # ---------------------------------------------------------
            # PRESENTATION DEBUGGING TOOL: RAW TEXT INSPECTOR
            # ---------------------------------------------------------
            st.markdown("---")
            with st.expander("🔍 Presentation Debugger: View Raw Extracted Document Text", expanded=False):
                st.write("This is the exact string payload generated by Docling and sent to GPT-4o:")
                raw_text_payload = values.get("raw_text", "")
                if raw_text_payload:
                    st.code(raw_text_payload, language="markdown")
                else:
                    st.caption("No raw text string metadata cached in current graph state.")
